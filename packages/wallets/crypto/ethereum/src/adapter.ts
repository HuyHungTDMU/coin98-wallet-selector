...
interface CryptoWallet extends EventEmitter {
  request(params: { method: string; params?: string | string[] | unknown }): Promise<string[] | string>;
}

interface CryptoWindow extends Window {
  deficonnectProvider?: CryptoWallet;
}

declare const window: CryptoWindow;

export interface CryptoWalletAdapterConfig {}

export const CryptoWalletName = 'Crypto.com' as WalletName<'Crypto.com'>;

export class CryptoWalletAdapterEthereum extends BaseMessageSignerWalletAdapterEVM {
  id = 'crypto_ether';
  chain = 'evm';
  ...
  private _wallet: CryptoWallet | null;
  ...
  constructor(config: CryptoWalletAdapterConfig = {}) {
    super();
  ...
  async autoConnect() {
    try {
      ...
      const wallet = window.deficonnectProvider!;
      let address: string;
