...
interface SafePalWallet extends EventEmitter {
  isSafePal?: boolean;
...
export class SafePalWalletAdapterEthereum extends BaseMessageSignerWalletAdapterEVM {
  id = 'SafePal_ether';
  chain = 'evm';
  name = SafePalWalletName;
  url = 'https://www.safepal.com/en/extension';
  icon = iconUrl;
  ...
  async autoConnect() {
    try {
      ...
      const wallet = window.safepalProvider!;
      let address: string;
