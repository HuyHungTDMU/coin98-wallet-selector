...
interface BinanceWallet extends EventEmitter {
  chainId: string;

  isConnected(): boolean;

  switchNetwork(networkId: any): Promise<void>;

  request(params: { method: string; params?: string | string[] | unknown }): Promise<string[] | string>;
}

interface BinanceWalletWindow extends Window {
  BinanceChain?: BinanceWallet;
}

declare const window: BinanceWalletWindow;

export interface BinanceWalletAdapterConfig {}

export const BinanceWalletName = 'Binance Wallet' as WalletName<'Binance Wallet'>;

export type BinanceWalletChainType = {
  chainId: string;
  chainName: string;
};

export const BinanceWalletChainsSupported: BinanceWalletChainType[] = [
  {
    chainId: '0x38',
...
export class BinanceWalletAdapterEthereum extends BaseMessageSignerWalletAdapterEVM {
  id = 'binance_wallet_ether';
  chain = 'evm';
  ...
  async connect(
    ...
    try {
      ...
      const chainSupport = BinanceWalletChainsSupported.find(item => item.chainId === chainId);
      if (!chainSupport) throw new WalletNotConnectedError();

      try {
        address = (await wallet.request({ method: 'eth_requestAccounts' }))[0]!;
        await wallet.switchNetwork(chainSupport.chainName);
      } catch (error: any) {
        throw new WalletAccountError(error?.message, error);
      }
  ...
  async signTypedDataV4(msgParams: TypedMessageV4<any>): Promise<WalletReturnType<string, string>> {
    return { data: null, error: "The method 'eth_signTypedData_v4' is not supported.", isError: true };
  }

  async signTypedDataV3(msgParams: TypedMessageV3<any>): Promise<WalletReturnType<string, string>> {
    return { data: null, error: "The method 'eth_signTypedData_v3' is not supported.", isError: true };
  }
