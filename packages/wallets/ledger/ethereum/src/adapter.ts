...
export class LedgerAdapterEthereum extends BaseMessageSignerWalletAdapterEVM {
    ...
    async getAccount(connectionType: ConnectionType) {
        ...
        const isSupported = connectionType === ConnectionType.hid ? await TransportWebHID.isSupported() : TransportWebBLE.isSupported();
        if (!isSupported) {
        ...
        if (transport) {
            ...
            try {
                newTransport = connectionType === ConnectionType.hid ? await TransportWebHID.create() : await TransportWebBLE.create();
    ...
    async signTransaction(rawTxHex: string, resolution?: LedgerEthTransactionResolution | null | undefined): Promise<WalletReturnType<string[], string>> {
        try {
            const transport = this._transport;
    ...
    async signTypedDataV4(msgParams: TypedMessageV4<any>): Promise<WalletReturnType<string, string>> {
        try {
            ...
            try {
                const eth = new Eth(transport);
                const msg = msgParams;
                const signature = await eth.signPersonalMessage("44'/60'/0'/0/0", Buffer.from(JSON.stringify(msg)).toString("hex"));
                const signedHash = "0x" + signature.r + signature.s + signature.v.toString(16);
